...
switch (environment) {
    case "local" :
        nodeAddress = "http://127.0.0.1:8020/"
...
if(!["avalanche","dijets","fuji","near"].includes(network)){
...
exec(`graph deploy ${user}/${network}-${subgraph} ${subgraphPath}/manifests/${network}.yaml --debug --ipfs ${ipfsAddress} --node ${nodeAddress} --version-label ${versionLabel} --access-token ${accessToken}`, (error, stdout, stderr) => {
    if (error) {
        program.error(error)
        return;
    }
